...
def decrypt_skytale(ciphertext: str, key: int) -> str:
    ...
    plaintext = ''.join(matrix[i][j]
                        for j in range(num_rows) for i in range(key))
...
if __name__ == "__main__":
    assert encrypt_skytale("WICHTIGENACHRICHTENKOMMENMORGEN",
                           8) == "WNTNIAEMCCNOHHKRTROGIIMEGCMNEHE"
    assert decrypt_skytale("WNTNIAEMCCNOHHKRTROGIIMEGCMNEHE",
                           8) == "WICHTIGENACHRICHTENKOMMENMORGEN"

    for i in range(10):
        plaintext = generate_random_str(random.randint(1, 100))
        key = random.randint(1, 100)
        assert decrypt_skytale(encrypt_skytale(
